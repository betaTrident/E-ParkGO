...
- [x] Draft ADR capturing D1–D7 and rejected alternatives.
- [x] Update `PLAN.md §0.2` to authorize Phase S0→S1 (do not start S1 until §0.2 points here).
- [x] Create evidence file with status table (S0…S6 = PENDING).

### Gate

- [x] ADR merged/committed; §0.2 playbook path equals this file; evidence skeleton exists.
...
| MODIFY | `tests/e2e/payment-exit.spec.ts` | Entry helper without space |
| MODIFY | `src/features/tickets/service.ts` | Nullable space join for pool entries (amend 2026-08-02) |
| MODIFY | `src/components/tickets/ticket-view.tsx` | Display capacity-pool entry when no bay (amend 2026-08-02) |
| GENERATE | `src/lib/supabase/database.types.ts` (via Supabase gen or patch) | After migration |
...
- [x] Write failing pgTAP for capacity full / entry without space.
- [x] Migration: capacities + replace `create_parking_entry` + snapshot metrics.
- [x] GREEN pgTAP; regenerate types.
- [x] RED/GREEN unit + update entry UI; update E2E helpers.
- [x] Admin settings: edit capacities.
- [x] Dashboard KPIs show pool free counts (real data only).
...
- [x] Full pool rejects entry with stable error code.
...
- [x] RED pgTAP: payment without shift succeeds; unauthenticated fails.
- [x] Replace `record_parking_payment` body; keep idempotency + exact amount + quote expiry checks.
- [x] Update payment UI + E2E (no shift).
- [x] Optional: staff cash totals RPC for admin later.
- [x] Security pass: ensure staff can only pay at own location (existing RLS/RPC guards).
...
- [x] Payment audit rows still include actor.
- [x] Evidence S2 PASS.
...
- [x] RED pgTAP for settle+exit atomicity and replay.
- [x] Migration + GREEN.
- [x] Build `ExitCheckout` UI; wire scanner → checkout.
- [x] Redirect old payment/confirm pages.
- [x] Rates UI: lost-ticket penalty help text (“Applied when processing a lost ticket”).
- [x] E2E happy path without visiting `/shifts` or separate confirm.
