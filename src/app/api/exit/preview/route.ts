import {
  containsForbiddenExitField,
  exitPreviewRequestSchema,
...
export async function POST(request: Request) {
  ...
  if (containsForbiddenExitField(body as Record<string, unknown>)) {
    return apiError(
      'VALIDATION_FAILED',
  ...
  const parsed = exitPreviewRequestSchema.safeParse({
  ...
  const result = await calculateExitPreview(parsed.data)
  ...
  if (!result.success || !result.data) {
    ...
    return apiError(
      'INTERNAL_ERROR',
      result.error ?? 'Unable to calculate the exit preview.',
      500,
      correlationId,
    )
  }
