import {
  containsForbiddenSettleField,
  settleCashAndExitRequestSchema,
} from '@/features/exit/schemas'
...
export async function POST(request: Request) {
  ...
  if (containsForbiddenSettleField(body as Record<string, unknown>)) {
  ...
  if (!result.success) {
    ...
    return apiError(
      'INTERNAL_ERROR',
      'error' in result ? result.error : 'Unable to settle and complete exit.',
      500,
      correlationId,
    )
  }
  ...
  if (!result.data) {
    return apiError('INTERNAL_ERROR', 'Unable to settle and complete exit.', 500, correlationId)
  }
